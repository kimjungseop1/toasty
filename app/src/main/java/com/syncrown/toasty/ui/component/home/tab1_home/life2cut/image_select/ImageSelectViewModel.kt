...
class ImageSelectViewModel : BaseViewModel() {
    ...
    fun addSelectedImage(imageInfo: ImageInfo, context: Context) {
        _selectedImages.value?.let {
            if (it.size >= 2) {
                Toast.makeText(context, "2개까지 추가 가능합니다.", Toast.LENGTH_SHORT).show()
                return
            }
            it.add(imageInfo)
    ...
    // 선택된 이미지를 삭제하는 함수
    fun removeSelectedImage(imageInfo: ImageInfo) {
        _selectedImages.value?.let {
            it.remove(imageInfo)
            _selectedImages.value = it
        }
    ...
    // 현재 선택되어있는 갯수 반환
    fun currentSelectCnt(): Int {
        return _selectedImages.value?.size ?: 0
    }
}