package com.syncrown.arpang.ui.component.home.tab5_more.scrap.adapter
...
class ScrapGridItemAdapter(
    private val context: Context,
    private val items: ArrayList<ResponseScrapContentDto.Root>?,
    ...
    inner class SingleSpanViewHolder(private val binding: ItemShareSingleBinding) :
        ...
        fun bind(item: ResponseScrapContentDto.Root, position: Int) {
            Glide.with(context)
                .load(item.img_url)
    ...
    fun updateData(newData: ArrayList<ResponseScrapContentDto.Root>) {
        items?.clear()
        items?.addAll(newData)
        notifyDataSetChanged()
    }

    fun addMoreData(newData: ArrayList<ResponseScrapContentDto.Root>) {
        if (newData.size != 0) {
