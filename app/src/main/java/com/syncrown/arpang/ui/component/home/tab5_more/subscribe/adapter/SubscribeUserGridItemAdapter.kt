...
class SubscribeUserGridItemAdapter(
    private val context: Context,
    private val items: ArrayList<ResponseSubscribeUserContentListDto.Root>?,
    ...
    inner class SingleSpanViewHolder(private val binding: ItemShareSingleBinding) :
        ...
        fun bind(item: ResponseSubscribeUserContentListDto.Root, position: Int) {
            Glide.with(context)
                .load(item.img_url)
    ...
    fun addMoreData(newData: ArrayList<ResponseSubscribeUserContentListDto.Root>) {
        if (newData.size != 0) {
